...
const EditableRequestCard = ({ element, allowed, children }) => {
    const divClass = (element.categoryId !== -1 && !allowed) ? "w3-panel w3-border w3-pale-yellow w3-round-large" : "w3-panel w3-border w3-light-gray w3-round-large"
    const mainView =
        (allowed) ? (
            ...
            (element.categoryId !== -1) ? (
                <AlertBoxNote
                    ...
                    context={"이러한 사진을 올린다면 법적 조치 가능하오니 자제 부탁 드립니다. :)"}
                />
            ) : (
                <AlertBoxNote
                    id={"has_no_category"}
    ...
    const imageView = (allowed && element.categoryId !== -1) ?
        (
